...
case class Sort(
  ...
  override def operatorMatch(plan: SparkPlan):Boolean = plan match{
    case to: Sort =>
      this.global == to.global &&
        !sortOrder.zipWithIndex.exists(x => !x._1.equals(to.sortOrder(x._2)))
    case _ => false
  }
  ...
  protected override def doExecute(): RDD[InternalRow] = attachTree(this, "sort") {
    child.execute().mapPartitions( { iterator =>
      ...
      val oldIter = iterator.map(_.copy()).toArray.sorted(ordering).iterator
      mapWithReuse[InternalRow](oldIter, row => row) //zengdan
...
case class ExternalSort(
  ...
  protected override def doExecute(): RDD[InternalRow] = attachTree(this, "sort") {
    child.execute().mapPartitions( { iterator =>
      ...
      mapWithReuse[InternalRow](CompletionIterator[InternalRow, Iterator[InternalRow]](baseIterator, sorter.stop()),
        row => row) //zengdan
    }, preservesPartitioning = true)
  }
...
case class TungstenSort(
  ...
  override def operatorMatch(plan: SparkPlan):Boolean = plan match{
    case to: TungstenSort =>
      this.global == to.global && !sortOrder.zipWithIndex.exists(x => !x._1.equals(to.sortOrder(x._2))) &&
        this.testSpillFrequency == to.testSpillFrequency
    case _ => false
  }
  ...
  protected override def doExecute(): RDD[InternalRow] = {
    ...
    def executePartition(
      ...
      mapWithReuse[InternalRow](sortedIterator, row => row) //zengdan
    }
