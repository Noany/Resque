...
case class HiveTableScan(
  ...
  //zengdan
  def compareOptionExpression(expr1: Seq[Expression], expr2: Seq[Expression]): Boolean = {
    if(expr1.size != expr2.size){
      false
    }else {
      compareExpressions(expr1.map(_.transformExpression()), expr2.map(_.transformExpression()))
    }
  ...
  protected override def doExecute(): RDD[InternalRow] = if (!relation.hiveQlTable.isPartitioned) {
    hadoopReader.makeRDDForTable(relation.hiveQlTable).mapPartitions{iter =>
      mapWithReuse[InternalRow](iter, row => row)
    }
  } else {
    hadoopReader.makeRDDForPartitionedTable(
      prunePartitions(relation.getHiveQlPartitions(partitionPruningPred))).mapPartitions { iter =>
      mapWithReuse[InternalRow](iter, row => row)
    }
  }
